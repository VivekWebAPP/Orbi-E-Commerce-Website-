...
const SignIn = () => {
  ...
  const { setauth, loginToken } = context;
  ...
  const handleToastError = () => {
    toast.error('Sign In Failed Please Re-enter The Details')
  }
  ...
  const handleSignUp = async (e) => {
    ...
    // ============== Getting the value ==============
    if (email && password) {
      const data = loginToken(name, email, password);
      data.then((data) => {
        if (data) {
          handleToastSuccess();
        ...
        handleToastError();
        setErrName('Enter correct name');
        setErrEmail('Enter correct email');
        setErrPassword('Enter correct password');
      })
      if(data){
        handleToastSuccess();
      }
